...
int	get_next_word(int fd, char **word, char *separator)
{
	...
	{
		buf[ret] = '\0';
		cast = (char *)current->content;
		if (!ft_strappend(&cast, buf))
			return (-1);
		if (ft_strpbrk(buf, separator))
			break ;
	}
	free(buf);
	if (!current->content || (ret < BUFF_SIZE && ft_strlen(current->content)))
		return (0);
	*word = ft_strdupchr(current->content, *ft_strpbrk(buf, separator));
